...
export const appendNewMessage = (data)=>{
    const {isAuthor,recieverSocketId,authorSocketId} = data;
    console.log(data);
    if(isAuthor)
    {
        appendNewMessageToChatHistory(recieverSocketId,data);
    }
    else{
...
const appendNewMessageToChatHistory = (userSocketId,data)=>{
    ...
    const userChatHistory = chatHistory.find(hist=> hist.socketId === userSocketId);
    if(userChatHistory)
    {
        ...
        const newChatHistory = [
            ...chatHistory.filter((hist)=> hist.socketId !== userSocketId),
            newUserChatHistory,
        ];
